...
public class DataItemDataType 
{
	...
	public static class Map extends Mapper<LongWritable,Text,IntWritable, Text> 
	{
		protected void map(LongWritable key, Text value,Mapper.Context context) throws IOException, InterruptedException 
	    {
			...
			//System.out.println("freqpattern:"+freqPattern);
	...
	public void getDataItemDataType(String dataset,String pattern) throws ClassNotFoundException, IOException, InterruptedException
	{
		 Configuration conf = new Configuration();
		    Job job = new Job(conf);
		    job.setJarByClass(DataItemDataType.class);
		    job.setOutputKeyClass(IntWritable.class);
		    job.setOutputValueClass(Text.class);
			EnhancedMetaDataGenerator emg=new EnhancedMetaDataGenerator();
			String output=emg.output;
		    job.setMapperClass(DataItemDataType.Map.class);
		    job.setReducerClass(DataItemDataType.Reduce.class);
		    freqPattern=pattern;
		    job.setInputFormatClass(TextInputFormat.class);
		    job.setOutputFormatClass(TextOutputFormat.class);

		    FileInputFormat.addInputPath(job, new Path(dataset));
		    FileOutputFormat.setOutputPath(job, new Path(output+"/Func5"));
